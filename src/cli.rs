...
pub struct Cli {
    ...
    /// Global until (used by validate/prep/land). Accepts 0, a local PR number, or a group selector
    #[arg(
        long,
        global = true,
...
mod tests {
    ...
    fn global_until_help_mentions_validate() {
        ...
        assert!(help.contains("used by validate/prep/land"));
    }
    ...
    fn absorb_help_text_mentions_pr_groups_and_example_flow() {
        ...
        assert!(long_about.contains("Run `spr absorb` from `user-spr/alpha`."));
        assert!(long_about.contains(
            "Result: the 2 new commits are folded into the `pr:alpha` group on the owning stack branch, and absorb still scans `pr:beta` and `pr:gamma` for their own append-only tails."
        ));
